...
class HighLevel:
    def __init__(self, symbol, interval, limit, dif_time=10, dif_percent=0.0005, vol_percent=0.01, h_l=1,
        ...
        self.vol_percent = vol_percent   # percent of etremum candle
        self.extremum = []
        self.can_couples = [[], []]
    ...
    def level_break(self):
            ...
            for ex in self.extremum[level[1][3] + 1:]:
                ...
                if self.h_l == 1 and ex[1] > lvl_max:
                    ...
                    for ind, lvl_break in enumerate(lvl_breaks):
                        if float(lvl_break[2]) > lvl_max:
                            count_m += 1
                            counter = 0
                            for i, lvl in enumerate(lvl_breaks[ind+1:]):
                                if lvl[3] >= lvl_break[2]:
                                    continue
                                if counter:
                                    break
                                else:
                                    counter += 1
                                    break_percent = (float(lvl_breaks[ind+i][2]) - lvl_max) / float(lvl_breaks[ind+i][2]) * 100
                                    summa += break_percent
                                    count += 1
                                    self.breaks[self.h_l - 1].append(
                                        [level, datetime.datetime.fromtimestamp(lvl_breaks[ind+i][0] // 1000),
                                         lvl_breaks[ind+i][2], break_percent])
                        if count_m:
                            break
                ...
                if self.h_l == 2 and ex[2] < lvl_min:
                    ...
                    lvl_breaks = client.futures_historical_klines(self.symbol, interval='1m',
                                                                  ...
                                                                  end_str=ex[0] + self.candle_time_secs * 1000)
                    for ind, lvl_break in enumerate(lvl_breaks):
                        if float(lvl_break[3]) < lvl_min:
                            count_m += 1
                            counter = 0
                            for i, lvl in enumerate(lvl_breaks[ind+1:]):
                                if lvl[2] <= lvl_break[3]:
                                    continue
                                if counter:
                                    break
                                else:
                                    counter += 1
                                    break_percent = abs(lvl_min - float(lvl_breaks[ind+i][3])) / lvl_min * 100
                                    self.breaks[self.h_l - 1].append(
                                        [level, datetime.datetime.fromtimestamp(lvl_breaks[ind+i][0] // 1000),
                                         lvl_breaks[ind+i][3], break_percent])
                                    summa += break_percent
                                    count += 1
                        if count_m:
                            break
        ...
        print('break:', summa/count)
        print(self.breaks)
        print('===========')
        print(self.intouchable)


BTC_level = HighLevel('BTCUSDT', '1h', 1500, dif_time=5, dif_percent=0.001, vol_percent=0.0045, h_l=1,
                      candle_time_secs=3600, laps=10)
BTC_level.print()
print('===========')
